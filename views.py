...
def valide(request):
    ...
    if(request.method=="POST"):
        formben=bene(request.POST)
        if formben.is_valid():
            benf=beneficiaire.objects.all().filter(id=formben.cleaned_data['id'])
            titulaire = benf.values('id')[0]['id']
            tit = beneficiaire.objects.get(id=titulaire)
            tit.first_name=formben.cleaned_data['first_name']
            tit.last_name=formben.cleaned_data['last_name']
            tit.email=formben.cleaned_data['email']
            tit.telephone=formben.cleaned_data['telephone']
            tit.motMairie=formben.cleaned_data['motMairie']
            tit.carteDonnee=formben.cleaned_data['carteDonnee']
            tit.presenceDistribution=formben.cleaned_data['presenceDistribution']
            tit.save()
            redirect('succeed')
        else:
            print("Error")
            print(formben.errors)
    else:
        formben=bene()
    return render(request,'authenticate/validé.html',{'bene':all_entries,'form':formben})

def delbenef(request,id):
    benee = beneficiaire.objects.all().filter(id=id).delete()
    return redirect('beneto')

def deletetitulaire(request, id):
    beneficiaire.objects.all().filter(id=id).delete()
